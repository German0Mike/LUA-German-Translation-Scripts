...
print("->LUA-Skript: Alle Gruppenmitglieder herbeirufen (.summonall)...geladen!")
--
	...
	if command == 'summonall' then
		...
		if has_value(maps, mapId) then
			...
			if not player:IsInCombat() then
				...
				chatHandler:SendSysMessage("Herbeirufen geht nicht in einen Kampf.")
			end
			return false
		else
			chatHandler:SendSysMessage("Beschwören ist hier nicht erlaubt.")
		end
		return false
	end
end
