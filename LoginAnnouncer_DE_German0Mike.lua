...
local function LoginAnnouncer(eventid, delay, repeats, player)
    ...
    if GameMasterAnnouncement and player:GetGMRank() >= 1 then
            msg = icons.logo ..strings.serverName ..icons.gameMaster .."|cff2ECCFA" ..player:GetName() .."|r " .. "ist online.|r"
    else
        if player:GetGMRank() < 1 then
            msg = icons.logo ..strings.serverName ..icons.faction[player:GetTeam()] ..strings.colorClass[player:GetClass()] ..player:GetName() .."|r "  .."ist online.|r"
        end
    end
    if msg then
