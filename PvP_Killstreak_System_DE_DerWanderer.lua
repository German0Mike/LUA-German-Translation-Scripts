...
╚══════════════════════════════════════════╝
--]]
print("->LUA-Skript: Killstreak...geladen!")
KILL = {}

TokenID = 40753 --Emblem der Ehre
	...
	if(player ~= victim) then
		if(map ~= 30 and  map ~= 489 and map ~= 529 and map ~= 466 and map ~= 607 and map ~= 628) then
			...
			if (KILL[player:GetName()] == nil) then
				...
				player:SendBroadcastMessage("Du hast folgendem Spieler auf deinem Gewissen: |cFF90EE90"..victim:GetName().."")
				player:SendBroadcastMessage("Mordserie: |cFF90EE90"..KILL[player:GetName()].killstreak.."")
				victim:SendBroadcastMessage("Du wurdest umgebracht von: |cFF90EE90"..player:GetName().."")
				...
				if (PlayerCampCheck(player, victim) == true) then
					...
					elseif (KILL[player:GetName()].killstreak == 10 or KILL[victim:GetName()].killstreak == 10) then
						...
						for k, v in pairs(plrs) do
							v:SendAreaTriggerMessage("|cFFff0000"..player:GetName().."|cffFFCD00 ist mit |cff076C02GOOD "..KILL[player:GetName()].killstreak.."|cffFFCD00 Morde in Serie.")
						end	
